	...
	function mfn_ID( $param = false )
	{
		...
		if (function_exists('is_woocommerce') && is_woocommerce() ) {
			...
			if( is_product() ){
				...
				if( get_post_meta($post_id, 'mfn_product_cat_template') && get_post_status( get_post_meta( $post_id, 'mfn_product_cat_template', true ) ) == 'publish' ){
					return get_post_meta($post_id, 'mfn_product_cat_template', true); // all categories product template
				}

				if( get_post_meta($post_id, 'mfn_product_tag_template') && get_post_status( get_post_meta( $post_id, 'mfn_product_tag_template', true ) ) == 'publish' ) {
					return get_post_meta($post_id, 'mfn_product_tag_template', true); // all tags product template
				}

				if( get_post_meta($post_id, 'mfn_product_template') && get_post_status( get_post_meta( $post_id, 'mfn_product_template', true ) ) == 'publish' ){
				...
				}elseif( (is_product_category() || is_product_tag()) && isset($qo->term_id) && !empty(get_term_meta($qo->term_id, 'mfn_shop_template', true)) && get_term_meta($qo->term_id, 'mfn_shop_template', true) == 'excluded' ){
...
if (! function_exists('mfn_template_part_ID')) {
	function mfn_template_part_ID( $type, $id = false ){
		...
		//if( !empty( $_GET['visual'] ) || ( get_post_type($post->ID) == 'template' && get_post_meta($post->ID, 'mfn_template_type', true) != $type ) ) return false;
		...
		if( is_singular() || is_search() ){
			...
			if( get_post_meta( $post_id, 'mfn_'.$type.'_post_excluded', true ) ) return;
			if( $single && is_numeric( $single ) && get_post_status( $single ) == 'publish' && get_post_type( $single ) == 'template' )
				return $single;

			// post type all
			$posttype = get_option( 'mfn_'.$type.'_'.$post_type.'_single' );
			if( get_option( 'mfn_'.$type.'_'.$post_type.'_page_single_excluded' ) ) return;
