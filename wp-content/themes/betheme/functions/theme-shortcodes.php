	...
	function sc_header_icon($attr){
		...
		if(in_array($type, array('default', 'mail', 'tel'))){
			...
			if(function_exists('is_woocommerce')){
				$additional_html .= '<div class="mfn-header-login woocommerce '. ( is_user_logged_in() ? "mfn-header-modal-nav" : "mfn-header-modal-login" ) .'">';
			...
			if( empty($image) && empty($icon) ) $icon_html = '<svg width="26" viewBox="0 0 26 26" aria-label="Search icon"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><circle class="path" cx="11.35" cy="11.35" r="6"></circle><line class="path" x1="15.59" y1="15.59" x2="20.65" y2="20.65"></line></svg>';
		}
		...
		$output .= '<a '.$target.' href="'.$attr_link.'" class="'. $classes .'">';
...
if (! function_exists('sc_header_burger')) {
	function sc_header_burger($attr){
		...
		// tmpl menu
		$output .= '<div class="mfn-header-tmpl-menu-sidebar mfn-header-tmpl-menu-sidebar-'.$menu_pos.'"><div class="mfn-header-tmpl-menu-sidebar-wrapper">';
...
if (! function_exists('sc_product_additional_information')) {
	function sc_product_additional_information($attr, $product = false)
	{
		...
		ob_start();
		if($attr['title'] == 1) echo '<h3>'. __( 'Additional information', 'woocommerce' ) .'</h3>';
...
if (! function_exists('sc_payment_methods')) {
	function sc_payment_methods($attr)
	{
		...
		$output .= '<ul class="payment-methods-list element_classes '.$classes.'">';
...
if (! function_exists('sc_image')) {
	function sc_image($attr, $content = null)
	{
		...
		$is_mask_shape_enabled = $mask_shape_type !== '0' ? true : false;
		...
		if( strpos( $src, '.gif' ) !== false ){
			$image_output_escapes = '<img class="scale-with-grid" '. $img_style .' src="'. esc_url($src) .'" alt="'. esc_attr($alt) .'" title="'. esc_attr($title) .'" '. $width_escaped .' '. $height_escaped .'/>';
		}else{
			$image_output_escapes = '<img class="scale-with-grid" '. $img_style .' src="'. esc_url($src) .'" alt="'. esc_attr($alt) .'" title="'. esc_attr($title) .'" '. $width_escaped .' '. $height_escaped .' '. $srcset .'/>';
		}
...
if (! function_exists('sc_clients_slider')) {
	function sc_clients_slider($attr, $content = null)
	{
		...
		if ($clients_query->have_posts()) {
			$output  = '<div class="clients_slider" data-client-per-slide="'. intval($per_slide) .'">';
