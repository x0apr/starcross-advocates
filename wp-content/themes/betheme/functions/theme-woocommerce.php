...
if ( ! function_exists( 'woocommerce_header_add_to_cart_fragment' ) ) {
	function woocommerce_header_add_to_cart_fragment( $fragments ){
		if(mfn_opts_get('shop-icon-count-if-zero') == 0 && WC()->cart->get_cart_contents_count() == 0){
		...
		ob_start();
		echo '<span class="header-cart-count">'. esc_html( WC()->cart->get_cart_contents_count() ) .'</span>';
		$fragments['.header-cart-count'] = ob_get_clean();

		ob_start();
		echo '<p class="header-cart-total">'. wp_strip_all_tags( WC()->cart->get_cart_total() ) .'</p>';
