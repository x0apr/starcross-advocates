...
echo '<div id="Side_slide" class="'. esc_attr($side_class) .'" data-width="'. esc_attr(mfn_opts_get('responsive-side-slide-width', 250)) .'" aria-expanded="false" role="banner" aria-label="Responsive menu">';

// close button

echo '<div class="close-wrapper">';
	echo '<a href="#" class="close"><i class="icon-cancel-fine" aria-label="menu close icon"></i></a>';
echo '</div>';
...
echo '<div class="extras">';
	...
	echo '<div class="extras-wrapper" role="navigation" aria-label="Extras menu">';
		...
		if( $has_wishlist ){
			echo '<a id="wishlist_button" class="top-bar-right-icon top-bar-right-icon-wishlist" href="'. get_permalink( mfn_opts_get('shop-wishlist-page') ). '">';
				if( $wishlist_icon ){
					echo '<i class="'. $wishlist_icon .'" aria-label="wishlist icon"></i>';
				} else {
					echo '<svg width="26" viewBox="0 0 26 26" aria-label="wishlist icon"><defs><style>.path{fill:none;stroke:#333;stroke-width:1.5px;}</style></defs><path class="path" d="M16.7,6a3.78,3.78,0,0,0-2.3.8A5.26,5.26,0,0,0,13,8.5a5,5,0,0,0-1.4-1.6A3.52,3.52,0,0,0,9.3,6a4.33,4.33,0,0,0-4.2,4.6c0,2.8,2.3,4.7,5.7,7.7.6.5,1.2,1.1,1.9,1.7H13a.37.37,0,0,0,.3-.1c.7-.6,1.3-1.2,1.9-1.7,3.4-2.9,5.7-4.8,5.7-7.7A4.3,4.3,0,0,0,16.7,6Z"/></svg>';
				}
				...
				if ($active_lang) {
					...
					echo '<a class="lang-active" href="#">';
						...
						if (count($languages) > 1) {
							echo '<i class="icon-down-open-mini" aria-label="language dropdown menu icon"></i>';
						}
					echo '</a>';
				}
			}
...
if (mfn_opts_get('header-search')) {
	echo '<div class="search-wrapper">';
		echo '<form id="side-form" method="get" action="'. esc_url(home_url('/')) .'" role="search" aria-label="Side Slide Search">';
			...
			echo '<a class="submit" href="#"><i class="icon-search-fine" aria-label="search submit icon"></i></a>';
...
echo '<div class="lang-wrapper" role="navigation" aria-label="Translation Menu">';
...
echo '<div class="menu_wrapper" role="navigation" aria-label="Main Menu"></div>';
